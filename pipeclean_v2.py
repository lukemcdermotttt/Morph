...
from data.BragnnDataset import setup_data_loaders
...
def search(trial, cfg: dict) -> float:
    ...
    # Generate the model.
    model = blocks.CandidateArchitecture(trial).to(device)
    print(model)
    ...
    # Get the dataset.
    train_loader, valid_loader = setup_data_loaders(batch_size=128, IMG_SIZE=11, aug=1, num_workers=4, pin_memory=False, prefetch_factor=2)
